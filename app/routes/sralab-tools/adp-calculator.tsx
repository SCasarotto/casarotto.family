...
import { FormSubmit } from '~/components/sralab/FormSubmit';
...
const createHoursData = (d: CreateHoursData) => {
  ...
  const lookupKey =
    treatmentHours === 0.25
      ? 0.25 * 60
      : roundDownToNearestHalf(treatmentHours) * 60;
  const adpMinutes = scale?.[lookupKey] ?? 0;
  const startTime =
    previousIndex >= 0
      ? prevHourArray[previousIndex].endTime
      : startOfDayTime ?? 0;
    ...
    <div className='p-6'>
      ...
      <Form state={form} resetOnSubmit={false}>
        <InputRow>
          ...
          <InputWrapper>
            <Label name='startOfDay' htmlFor='startOfDay'>
              Start Time
            </Label>
            <DatePicker
              id='startOfDay'
