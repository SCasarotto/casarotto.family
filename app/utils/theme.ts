...
export const getButtonStyles = (variant?: 'primary' | 'secondary') => {
  const base =
    'w-full rounded-lg border transition duration-100 ease-in py-2 px-4';
  switch (variant) {
    case 'secondary':
      return `${base} text-gray-700 bg-white border-gray-300 hover:bg-gray-100/60`;
    case 'primary':
    default:
      return `${base} text-white bg-sra-brand-orange-500 border-none hover:bg-brand-orange-600`;
  }
};
